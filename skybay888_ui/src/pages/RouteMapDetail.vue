					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateRouteMap()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2RouteMaps = true">Add</base-button>
								              <modal :show.sync="modal2RouteMaps">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Route Map Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="RouteMapId" type="text" placeholder="Enter RouteMapId" v-model="routeMapDetails.routeMapId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Distance</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Distance" type="text" placeholder="Enter Distance" v-model="routeMapDetails.distance"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Duration</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Duration" type="text" placeholder="Enter Duration" v-model="routeMapDetails.duration"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateRouteMap() {
      ...
      const jsonData = JSON.stringify(this.routeMapDetails);
      const res = await RouteMapService.update(jsonData);
...
//        this.getAllRouteMaps();
    ...
    async getRouteMapDetails() {
      try {
			let response = await RouteMapService.get(this.routeMapId);
			this.routeMapDetails = response.data;
