					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateSeat()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Seats = true">Add</base-button>
								              <modal :show.sync="modal2Seats">
								  ...
								  <base-input label="SeatType" type="text" placeholder="Enter SeatType" v-model="seatToAdd.seatType"></base-input>
												                  </form>
								                </div>
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Seat Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="SeatId" type="text" placeholder="Enter SeatId" v-model="seatDetails.seatId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Seat Number</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="SeatNumber" type="text" placeholder="Enter SeatNumber" v-model="seatDetails.seatNumber"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Seat Type</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="SeatType" type="text" placeholder="Enter SeatType" v-model="seatDetails.seatType"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateSeat() {
      ...
      const jsonData = JSON.stringify(this.seatDetails);
      const res = await SeatService.update(jsonData);
...
//        this.getAllSeats();
    ...
    async getSeatDetails() {
      try {
			let response = await SeatService.get(this.seatId);
			this.seatDetails = response.data;
