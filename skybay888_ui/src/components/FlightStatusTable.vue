        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Flight Statuss</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalFlightStatuss = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalFlightStatuss">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add FlightStatus</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="FlightStatusId" type="text" placeholder="Enter FlightStatusId" v-model="flightStatusToAdd.flightStatusId"></base-input>
  <base-input label="StatusDescription" type="text" placeholder="Enter StatusDescription" v-model="flightStatusToAdd.statusDescription"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="flightStatuss" :row-key="record => record.FlightStatusId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <FlightStatusPictureView :flightStatuss="flightStatuss" />
          </div>
...
import FlightStatusPictureView from './FlightStatusPictureView.vue';


const flightStatussColumns = [
  "flightStatusId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    FlightStatusPictureView
  },

  data() {
    return {
      modalFlightStatuss: false,
        isTableView: true,

      columns: [
        {
          title: 'Flight Status Id',
		dataIndex: 'flightStatusId',
          visible: true,
          scopedSlots: { customRender: 'flightStatusId' },
          sorter: true
          //sorter: (a, b) => a.flightStatusId - b.flightStatusId,
          //sorter: (a, b) => a.flightStatusId.localeCompare(b.flightStatusId),
        },
        {
          title: 'Status Description',
		dataIndex: 'statusDescription',
          visible: true,
          scopedSlots: { customRender: 'statusDescription' },
          sorter: true
          //sorter: (a, b) => a.statusDescription - b.statusDescription,
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} flightStatuss`,
      },
      ...
      flightStatussTable: {
        columns: [...flightStatussColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'flightStatusId',           // Column to sort by
  ...
  methods: {
    ...
    async renderFlightStatussTable() {
      ...
      let flightStatussTableData = [];
      for (let i = 0; i < this.flightStatuss.length; i++) {
        flightStatussTableData.push({
          id: i,
          flightStatusId: this.flightStatuss[i].flightStatusId,
          year: this.flightStatuss[i].year,
          date: this.flightStatuss[i].date,
          competitionId: this.flightStatuss[i].competitionId,
          flightStatusId: this.flightStatuss[i].flightStatusId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-flight-statuss',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForFlightStatussChanged', this.searchQuery);
		//this.renderFlightStatussTable();
    },
	...
	async handleAddSubmitted() {
      this.modalFlightStatuss = false;

      const currentDate = new Date().getTime();
      this.flightStatusToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.flightStatusToAdd);
      console.log(jsonData);
      
      const res = await FlightStatusService.addFlightStatus(jsonData);
