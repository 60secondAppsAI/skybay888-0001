        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Loungess</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalLoungess = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalLoungess">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Lounges</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="LoungesId" type="text" placeholder="Enter LoungesId" v-model="loungesToAdd.loungesId"></base-input>
  <base-input label="LoungeName" type="text" placeholder="Enter LoungeName" v-model="loungesToAdd.loungeName"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="loungess" :row-key="record => record.LoungesId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <LoungesPictureView :loungess="loungess" />
          </div>
...
import LoungesService from "../services/LoungesService";
import { ASelect, ASelectOption, AButton, Table, Pagination } from "ant-design-vue";
import LoungesPictureView from './LoungesPictureView.vue';


const loungessColumns = [
  "loungesId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'loungesId',
          visible: true,
          scopedSlots: { customRender: 'loungesId' },
          sorter: true
          //sorter: (a, b) => a.loungesId - b.loungesId,
          //sorter: (a, b) => a.loungesId.localeCompare(b.loungesId),
        },
        {
          title: 'Lounge Name',
		dataIndex: 'loungeName',
          visible: true,
          scopedSlots: { customRender: 'loungeName' },
          sorter: true
          //sorter: (a, b) => a.loungeName - b.loungeName,
          //sorter: (a, b) => a.loungeName.localeCompare(b.loungeName),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} loungess`,
      },
      ...
      loungessTable: {
        columns: [...loungessColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'loungesId',           // Column to sort by
  ...
  methods: {
    ...
    async renderLoungessTable() {
      ...
      let loungessTableData = [];
      for (let i = 0; i < this.loungess.length; i++) {
        loungessTableData.push({
          id: i,
          loungesId: this.loungess[i].loungesId,
          year: this.loungess[i].year,
          date: this.loungess[i].date,
          competitionId: this.loungess[i].competitionId,
          loungesId: this.loungess[i].loungesId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-loungess',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForLoungessChanged', this.searchQuery);
		//this.renderLoungessTable();
    },
	...
	async handleAddSubmitted() {
      this.modalLoungess = false;

      const currentDate = new Date().getTime();
      this.loungesToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.loungesToAdd);
      console.log(jsonData);
      
      const res = await LoungesService.addLounges(jsonData);
