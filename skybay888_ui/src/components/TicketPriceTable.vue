        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalTicketPrices = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalTicketPrices">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add TicketPrice</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="TicketPriceId" type="text" placeholder="Enter TicketPriceId" v-model="ticketPriceToAdd.ticketPriceId"></base-input>
  <base-input label="Price" type="text" placeholder="Enter Price" v-model="ticketPriceToAdd.price"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="ticketPrices" :row-key="record => record.TicketPriceId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <TicketPricePictureView :ticketPrices="ticketPrices" />
          </div>
...
import TicketPricePictureView from './TicketPricePictureView.vue';


const ticketPricesColumns = [
  "ticketPriceId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    TicketPricePictureView
  },

  data() {
    return {
      modalTicketPrices: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'ticketPriceId',
          visible: true,
          scopedSlots: { customRender: 'ticketPriceId' },
          sorter: true
          //sorter: (a, b) => a.ticketPriceId - b.ticketPriceId,
          //sorter: (a, b) => a.ticketPriceId.localeCompare(b.ticketPriceId),
        },
        {
          title: 'Price',
		dataIndex: 'price',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} ticketPrices`,
      },
      ...
      ticketPricesTable: {
        columns: [...ticketPricesColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'ticketPriceId',           // Column to sort by
  ...
  methods: {
    ...
    async renderTicketPricesTable() {
      ...
      let ticketPricesTableData = [];
      for (let i = 0; i < this.ticketPrices.length; i++) {
        ticketPricesTableData.push({
          id: i,
          ticketPriceId: this.ticketPrices[i].ticketPriceId,
          year: this.ticketPrices[i].year,
          date: this.ticketPrices[i].date,
          competitionId: this.ticketPrices[i].competitionId,
          ticketPriceId: this.ticketPrices[i].ticketPriceId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-ticket-prices',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForTicketPricesChanged', this.searchQuery);
		//this.renderTicketPricesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalTicketPrices = false;

      const currentDate = new Date().getTime();
      this.ticketPriceToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.ticketPriceToAdd);
      console.log(jsonData);
      
      const res = await TicketPriceService.addTicketPrice(jsonData);
