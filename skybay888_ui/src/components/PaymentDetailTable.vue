        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalPaymentDetails = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPaymentDetails">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add PaymentDetail</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PaymentDetailId" type="text" placeholder="Enter PaymentDetailId" v-model="paymentDetailToAdd.paymentDetailId"></base-input>
  <base-input label="Amount" type="text" placeholder="Enter Amount" v-model="paymentDetailToAdd.amount"></base-input>
  <base-input label="PaymentDate" type="text" placeholder="Enter PaymentDate" v-model="paymentDetailToAdd.paymentDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="paymentDetails" :row-key="record => record.PaymentDetailId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PaymentDetailPictureView :paymentDetails="paymentDetails" />
          </div>
...
import PaymentDetailPictureView from './PaymentDetailPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'paymentDetailId',
          visible: true,
          scopedSlots: { customRender: 'paymentDetailId' },
          sorter: true
          //sorter: (a, b) => a.paymentDetailId - b.paymentDetailId,
          //sorter: (a, b) => a.paymentDetailId.localeCompare(b.paymentDetailId),
        },
        {
          title: 'Amount',
		dataIndex: 'amount',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} paymentDetails`,
      },
      ...
      sortBy: 'paymentDetailId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPaymentDetailsTable() {
      ...
      let paymentDetailsTableData = [];
      for (let i = 0; i < this.paymentDetails.length; i++) {
        paymentDetailsTableData.push({
          id: i,
          paymentDetailId: this.paymentDetails[i].paymentDetailId,
          year: this.paymentDetails[i].year,
          date: this.paymentDetails[i].date,
          competitionId: this.paymentDetails[i].competitionId,
          paymentDetailId: this.paymentDetails[i].paymentDetailId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-payment-details',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPaymentDetailsChanged', this.searchQuery);
		//this.renderPaymentDetailsTable();
    },
	...
	async handleAddSubmitted() {
      ...
      this.paymentDetailToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.paymentDetailToAdd);
      console.log(jsonData);
      
      const res = await PaymentDetailService.addPaymentDetail(jsonData);
