        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Delayss</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalDelayss = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalDelayss">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Delays</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="DelaysId" type="text" placeholder="Enter DelaysId" v-model="delaysToAdd.delaysId"></base-input>
  <base-input label="Reason" type="text" placeholder="Enter Reason" v-model="delaysToAdd.reason"></base-input>
  <base-input label="Duration" type="text" placeholder="Enter Duration" v-model="delaysToAdd.duration"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="delayss" :row-key="record => record.DelaysId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <DelaysPictureView :delayss="delayss" />
          </div>
...
import DelaysPictureView from './DelaysPictureView.vue';


const delayssColumns = [
  "delaysId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'delaysId',
          visible: true,
          scopedSlots: { customRender: 'delaysId' },
          sorter: true
          //sorter: (a, b) => a.delaysId - b.delaysId,
          //sorter: (a, b) => a.delaysId.localeCompare(b.delaysId),
        },
        {
          title: 'Reason',
		dataIndex: 'reason',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} delayss`,
      },
      ...
      delayssTable: {
        columns: [...delayssColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'delaysId',           // Column to sort by
  ...
  methods: {
    ...
    async renderDelayssTable() {
      ...
      let delayssTableData = [];
      for (let i = 0; i < this.delayss.length; i++) {
        delayssTableData.push({
          id: i,
          delaysId: this.delayss[i].delaysId,
          year: this.delayss[i].year,
          date: this.delayss[i].date,
          competitionId: this.delayss[i].competitionId,
          delaysId: this.delayss[i].delaysId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-delayss',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForDelayssChanged', this.searchQuery);
		//this.renderDelayssTable();
    },
	...
	async handleAddSubmitted() {
      this.modalDelayss = false;

      const currentDate = new Date().getTime();
      this.delaysToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.delaysToAdd);
      console.log(jsonData);
      
      const res = await DelaysService.addDelays(jsonData);
